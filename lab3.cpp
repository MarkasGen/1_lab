...

{
	...
	for (int i = 0; i < kiek; i++) {
		...
		float paz = 5.00;
		if (studentai.at(i).GP < paz) {
			vargs_failas << studentai.at(i).Vardas << std::setw(20) << studentai.at(i).Pavarde << std::setw(18) << studentai.at(i).GP << std::endl;
		}
	...
	for (int j = 0; j < kiek; j++) {
		float paz = 5.00;
		if (studentai.at(j).GP >= paz) {
			prot_failas << studentai.at(j).Vardas << std::setw(20) << studentai.at(j).Pavarde << std::setw(18) << studentai.at(j).GP << std::endl;
		}
	}
